...
module.exports = (req, res) => {
  ...
  let myModel = $db.download;
  let size = curPage * 5; //每次拿取5条数据
  let reg = new RegExp(searchText);

  if (curPage && searchText) {
    myModel.find({ file_creater: teaName, file_name: { $regex: reg } }, (err, result) => {
      error(res, err);
      let rel = result;
      let arr = [];
      ...
      myModel.find({ file_creater: teaName, file_name: { $regex: reg } }, (err, result) => {  //获取总页数
        error(res, err);
