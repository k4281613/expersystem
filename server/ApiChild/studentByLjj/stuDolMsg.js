...
module.exports = (req, res) => {
    ...
    if (curClassify && curPage) {
        myModel.find({ file_classify: curClassify }, null, { limit: size, skip: size - 5 }, (err, result) => { //查数据库
            error(res, err);
            let rel = result;
            let arr = [];
            ...
            myModel.find({ file_classify: curClassify }, (err, result) => {  //获取总页数
                error(res, err);
